...
barplot <- ggplot(plot_data, aes(x = category, y = proportion, fill = category)) +
  ...
  coord_cartesian(ylim = c(0,4.5e9))
barplot
...
library(ggplot2)
df <- data.frame(
  log_p_value = c(log_p_spliced, log_p_unspliced),
  Type = rep(c("Spliced", "Unspliced"), each = length(log_p_spliced))
)

# comparison the distribution
comparison_distribution <- ggplot(df, aes(x = log_p_value, fill = Type)) +
  geom_density(alpha = 0.5, bw = 0.6) +  # Adjust bw for smoothness
  geom_vline(xintercept = -log10(0.05), linetype = "dashed", color = "red", linewidth = 1) +  # Add vertical line
    annotate("text", x = -log10(0.05)+0.05, y = 0.5, label = "p-value = 0.05", color = "red", hjust = 0,size=5) +
  ...
  theme(
    ...
    plot.title = element_text(hjust = 0.5, vjust = 2),  # Centered & bold title
    legend.background = element_blank()  # Improve legend visibility
  )
comparison_distribution

library(ggvenn)

sig_spliced <- which(SigRMtest_res_spliced_p_value_median <= 0.05)
sig_unspliced <- which(SigRMtest_res_unspliced_p_value_median <= 0.05)

# Create a named list for the Venn diagram
venn_list <- list(
  Spliced = sig_spliced,
  Unspliced = sig_unspliced
)
ggvenn(venn_list,fill_alpha = 0.7,stroke_size = 0.5)
...
index_not_dominate <- which(SigRMtest_res_overall_pvalue_median<=0.05)
