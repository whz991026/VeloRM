---
title: "DART-seq_m6A_dataset_2"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{DART-seq_m6A_dataset_2}
...
### data download
https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE297551
### alignment
```{bash eval=FALSE}

# Base directories
input_base="/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR"
output_base="/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR"

# Extract SRR numbers into an array (skip the header line)
srr_list=($(find "${input_base}" -mindepth 1 -maxdepth 1 -type d -name "*SRR*" -exec basename {} \;))
...
for srr in "${srr_list[@]}"; do
    ...
    fasterq-dump \
        -e 8 \
        -O "${input_base}/${srr}" \
        "${input_base}/${srr}/${srr}.sra"
    # Run trim_galore in paired-end mode
    trim_galore \
        --paired \
        --cores 4 \
        --output_dir "${output_base}/${srr}" \
        "${input_base}/${srr}/${srr}_1.fastq" \
        "${input_base}/${srr}/${srr}_2.fastq"
done
...
# Run STAR for each SRR sample
for srr in "${srr_list[@]}"; do
    echo "Processing sample: $srr"
    STAR --runThreadN 1 \
    --outSAMtype BAM Unsorted \
    --readFilesIn \
        "${input_base}/${srr}/${srr}_1_val_1.fq" \
        "${input_base}/${srr}/${srr}_2_val_2.fq" \
...
# Run Samtools for each SRR sample
for srr in "${srr_list[@]}"; do
    echo "Processing sample: $srr"
...
srr_info <- list.dirs("/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/", recursive = FALSE, full.names = FALSE)
...
for (srr in srr_info) {
  # Load VCF file
  data <- read.vcfR(paste0("/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/", srr, "/", srr, "_calls.vcf"))
  ...
  if (nrow(data2) == 0) {
    ...
    saveRDS(NULL, paste0("/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/", srr, "/", srr, "SNP.rds"))
  } else {
    ...
    saveRDS(gr, paste0("/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/", srr, "/", srr, "SNP.rds"))
    ...
    saveRDS(gr_filtered, paste0("/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/", srr, "/", srr, "SNP_filtered.rds"))
  }
...
gr_all <- GRanges()
for (srr in srr_info) {
  gr <- readRDS(paste0("/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/", srr, "/", srr, "SNP_filtered.rds"))
  if (!is.null(gr)) {
    gr_all <- c(gr_all, gr)
  }
}
...
saveRDS(gr_all, "/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/SNP_all.rds")
...
gr_all<- readRDS('/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/SNP_all.rds')
...
for (srr in srr_info) {
  ...
  j <- j+1
  name <- srr
  path <- paste0("/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/",srr,"/")
...
control_srr_info <- c("SRR33626939","SRR33626940","SRR33626941","SRR33626942","SRR33626943",
"SRR33626944","SRR33626945","SRR33626946","SRR33626947","SRR33626948","SRR33626949")
test_srr_info <- c("SRR33626969","SRR33626970","SRR33626971","SRR33626972","SRR33626973","SRR33626976",
"SRR33626977","SRR33626978","SRR33626980","SRR33626981","SRR33626983")

# Initialize the spliced_list
spliced_list <- list()
j = 1
for (srr in control_srr_info) {
  name <- srr
  path <- paste0('/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/', srr,"/")
...
names(spliced_list) <- control_srr_info
saveRDS(spliced_list, "/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/spliced_m6A_control.rds")

# Initialize the spliced_list
spliced_list <- list()
j = 1
for (srr in test_srr_info) {
  name <- srr
  path <- paste0('/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/', srr,"/")
  spliced_list[[j]] <- readRDS(paste0(path, srr, "_mRNA_spliced.bam_frequency_all.rds"))
  j = j + 1
...
# Name the elements of spliced_list
names(spliced_list) <- test_srr_info
saveRDS(spliced_list, "/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/spliced_m6A_test.rds")
...
names(pre_list) <- control_srr_info
saveRDS(pre_list, "/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/pre_m6A_control.rds")
...
# Name the elements of isoform_list
names(pre_list) <- test_srr_info
saveRDS(pre_list, "/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/pre_m6A_test.rds")
...
names(isoform_list) <- control_srr_info
saveRDS(isoform_list, "/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/isoform_m6A_control.rds")
...
# Name the elements of pre_list
names(isoform_list) <- test_srr_info
saveRDS(isoform_list, "/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR/isoform_m6A_test.rds")
...
root_dir="/gpfs/work/bio/haozhewang17/data/dart_seq_22_data/SRR"
...
# Read metadata
error_rate_meta_spliced <- readRDS("D:/data/VeloRM_data/DART-seq_2/pre_process/error_rate_meta_spliced_YTH.rds")
error_rate_meta_pre <- readRDS("D:/data/VeloRM_data/DART-seq_2/pre_process/error_rate_meta_pre_YTH.rds")
...
# Read counts
reads_counts_spliced_YTHmut <- readRDS("D:/data/VeloRM_data/DART-seq_2/pre_process/reads_counts_spliced_YTHmut.rds")
reads_counts_spliced_YTH <- readRDS("D:/data/VeloRM_data/DART-seq_2/pre_process/reads_counts_spliced_YTH.rds")
...
reads_counts_pre_YTHmut <- readRDS("D:/data/VeloRM_data/DART-seq_2/pre_process/reads_counts_pre_YTHmut.rds")
reads_counts_pre_YTH <- readRDS("D:/data/VeloRM_data/DART-seq_2/pre_process/reads_counts_pre_YTH.rds")
...
saveRDS(index_error,"D:/data/VeloRM_data/DART-seq_2/pre_process/index_error.rds")
saveRDS(index,"D:/data/VeloRM_data/DART-seq_2/pre_process/index.rds")


reads_counts_isoform_YTHmut <- readRDS("D:/data/VeloRM_data/DART-seq_2/pre_process/reads_counts_isoform_YTHmut.rds")
reads_counts_isoform_YTH <- readRDS("D:/data/VeloRM_data/DART-seq_2/pre_process/reads_counts_isoform_YTH.rds")
...
saveRDS(test.list,"D:/data/VeloRM_data/DART-seq_2/pre_process/test.list.rds")
saveRDS(control.list,"D:/data/VeloRM_data/DART-seq_2/pre_process/control.list.rds")
saveRDS(res_preprocess,"D:/data/VeloRM_data/DART-seq_2/pre_process/res_preprocess.rds")

SNP <- readRDS("D:/data/VeloRM_data/DART-seq_2/origin/SNP_all.rds")
SNP_index <- SNP[index]
saveRDS(SNP_index,"D:/data/VeloRM_data/DART-seq_2/pre_process/SNP.rds")
```
### DESeq2 to test the differential methylation between the spliced mRNA and pre-mRNA
```{r eval=FALSE}
# Load input data
test.list <- readRDS("D:/data/VeloRM_data/DART-seq_2/pre_process/test.list.rds")
...
# Save the results
saveRDS(result, "D:/data/VeloRM_data/DART-seq_2/pre_process/res_DESeq2.rds")
# pre deseq2
...
saveRDS(res,"D:/data/VeloRM_data/DART-seq_2/pre_process/res_binomial.rds")
```
...
counts_data <- read.csv("D:/data/VeloRM_data/DART-seq_2/origin/bam_read_counts.csv", 
...
DESeq2_res_spliced_overall <- readRDS("D:/data/VeloRM_data/DART-seq_2/pre_process/res_DESeq2_spliced.rds")
spliced_index <- which(DESeq2_res_spliced_overall@listData[["log2FoldChange"]]>1)

DESeq2_res_unspliced_overall <- readRDS("D:/data/VeloRM_data/DART-seq_2/pre_process/res_DESeq2_pre.rds")
...
res_DESeq2 <- readRDS("D:/data/VeloRM_data/DART-seq_2/pre_process/res_DESeq2.rds")
...
res <- readRDS("D:/data/VeloRM_data/DART-seq_2/pre_process/res_binomial.rds")
res_DESeq2 <- readRDS("D:/data/VeloRM_data/DART-seq_2/pre_process/res_DESeq2.rds")
...
plot_data <- data.frame(log2FC = log2FC, 
                        ...
                        neg_log10_p = -log10(pvals))
set.seed(42)
plot_data_5000_sample <- plot_data[sample(1:dim(plot_data)[1],5000),]
