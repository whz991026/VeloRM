...
input_base="/gpfs/work/bio/haozhewang17/data/RNA-Seq_A-to-I_SRR_GSE99933/SRR"
gtf_file="/gpfs/work/bio/haozhewang17/data/VeloRM_2/mm39/mm39.knownGene.gtf"
...
base_dir = "/gpfs/work/bio/haozhewang17/data/RNA-Seq_A-to-I_SRR_GSE99933/SRR"
...
isoform_TPM <- read.table("C:/Users/Haozhe Wang/OneDrive/æ¡Œé¢/all_samples_TPM_matrix_isoform-a.txt", 
                      header = TRUE,        # First row contains column names
                      row.names = 1,
                      sep = "\t")           # Separator is a tab

# Read a tab-separated file (common for .txt exports)
spliced_TPM <- read.table("C:/Users/Haozhe Wang/OneDrive/æ¡Œé¢/all_samples_TPM_matrix_mRNA_spliced.txt", 
                          header = TRUE,        # First row contains column names
                          row.names = 1,
                          sep = "\t")           # Separator is a tab

# Read a tab-separated file (common for .txt exports)
pre_TPM <- read.table("C:/Users/Haozhe Wang/OneDrive/æ¡Œé¢/all_samples_TPM_matrix_pre-mRNA.txt", 
                          header = TRUE,        # First row contains column names
                          row.names = 1,
                          sep = "\t")           # Separator is a tab
...
We will get the following data in VeloRM_data/RNA-Seq_A-to-I/origin

Download Data

The following raw data files are required:

cell.colors.rds and cell_colors_data.csv

classification.csv

embedding.rds and embedding_data.csv
...
# Read metadata
error_rate_meta_spliced <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/error_rate_meta_spliced.rds")
error_rate_meta_pre <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/error_rate_meta_pre.rds")
index_error <- intersect(which(error_rate_meta_spliced <= 0.01), which(error_rate_meta_pre <= 0.01))



# Read counts
reads_counts_spliced<- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/reads_counts_spliced.rds")
...
reads_counts_pre<- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/reads_counts_pre.rds")
...
saveRDS(union(index_cell_discard_spliced,index_cell_discard_pre),"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/index_cell_discard.rds")
saveRDS(index_error,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/index_error.rds")
saveRDS(index,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/index.rds")


reads_counts_isoform <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/reads_counts_isoform.rds")
reads_counts_isoform <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/reads_counts_isoform.rds")
...
saveRDS(test.list,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/test.list.rds")
saveRDS(control.list,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/control.list.rds")
saveRDS(res_preprocess_0.5,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/res_preprocess_0.5.rds")
saveRDS(res_preprocess,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/res_preprocess.rds")
# 
SNP <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/origin/SNP_all.rds")
SNP_index <- SNP[index]
saveRDS(SNP_index,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/SNP_1159.rds")
```
...
We will get the following data in VeloRM_data/RNA-Seq_A-to-I/pre_process
...
test.list <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/test.list.rds")
...
saveRDS(res,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/res_binomial.rds")
...
counts_data <- read.csv("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/origin/bam_read_counts.csv", 
...
res_preprocess <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/res_preprocess.rds")
...
snp <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/SNP_1019.rds")
...
gtf_file <- "D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/origin/gencode.vM33.annotation.gtf"
...
saveRDS(all_distances_df,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/splicing_junction_analysis/all_distances_df.rds")


```
#### vis
```{r warning=FALSE}
 all_distances_df <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/splicing_junction_analysis/all_distances_df.rds")
...
snp_OR_post <- res_preprocess[["spliced_methylation_Beta_value_meta"]]
snp_OR_pre  <- res_preprocess[["unspliced_methylation_Beta_value_meta"]]
...
gtf_data <- import("D:/data/VeloRM_2/mm39/mm39.knownGene.gtf")
exons <- gtf_data[gtf_data$type == "exon"]
...
#自动平滑拟合图 (展示整体趋势)
p_smooth <- ggplot(plot_df, aes(x = log10_dist, y = OR, color = Type, fill = Type)) +
  ...
  theme_minimal() +
  theme(panel.grid.minor = element_blank())+coord_cartesian(ylim=c(0,1))
...
res_DESeq2 <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/res_DESeq2.rds")
...
res <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/res_binomial.rds")
res_DESeq2 <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/res_DESeq2.rds")
...
cor(res$log2OR[which(res$pvalue<=0.05&res_DESeq2$pvalue<=0.05)],res_DESeq2$log2FoldChange[which(res$pvalue<=0.05&res_DESeq2$pvalue<=0.05)], method = "spearman", use = "na.or.complete")
...
volcano_plot_binomial <- ggplot(plot_data, aes(x = log2FC, y = neg_log10_p)) +
  # Add light gray background for -1 < log2FC < 1 (non-significant FC region)
  annotate("rect", xmin = -1, xmax = 1, ymin = 0, ymax = 30, 
           fill = "gray90", alpha = 0.3) +
...
SNP_all <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/origin/SNP_all.rds")
SNP_1019 <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/SNP_1019.rds")
...
# the input data
test.list <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/test.list.rds")
control.list <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/control.list.rds")
...
saveRDS(res, "D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/res_A_to_I_velocity.rds")

```

### load the embedding (tsne)
```{r,warning=FALSE}
sra_table <- read.csv("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/origin/SraRunTable_E12.5.csv", 
                      header = TRUE, 
                      stringsAsFactors = FALSE)

# Extract the first column which contains SRR 

classification_table <- read.csv("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/origin/classification.csv")
...
embedding <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/origin/embedding.rds")
cell.colors <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/origin/cell.colors.rds")
...
```{r eval=FALSE}
library(VeloRM)
res <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/res_A_to_I_velocity.rds")
res_preprocess <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/res_preprocess.rds")
# the input data
test.list <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/test.list.rds")
control.list <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/pre_process/control.list.rds")
...
saveRDS(vis_umap_Beta_emb_Beta_linear_index,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/vis_umap_Beta_emb_Beta_linear_index.rds")
saveRDS(vis_umap_Beta_emb_Beta_linear_index_grid,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/vis_umap_Beta_emb_Beta_linear_index_grid.rds")


diffusion_density_plot_tsne <- diffusion_density_plot(umap_results[,1:2],vis_umap_Beta_emb_Beta_linear_index[[2]]$tp,forward = TRUE,2500,scale="log")

saveRDS(diffusion_density_plot_tsne,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/diffusion_density_plot.rds")


arrow.scale=3; cell.alpha=0.4; cell.cex=1; fig.height=4;

vis_umap_Beta_emb_Beta_linear_index  <- show.velocity.on.embedding.cor(umap_results[,1:2],
             res[[3]][["current"]]$M[,index],
             res[[3]][["projected"]]$M[,index],
             res[[3]][["delta"]]$M[,index],
             cell.colors=cell.colors,n=200,scale='log',
             cex=cell.cex,arrow.scale=arrow.scale,arrow.lwd=0.5,rm_uniform="mode1")
vis_umap_Beta_emb_Beta_linear_index[[1]]
...
saveRDS(vis_umap_Beta_emb_Beta_linear_index,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/vis_umap_Beta_emb_Beta_linear_index_full_site.rds")
saveRDS(vis_umap_Beta_emb_Beta_linear_index_grid,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/vis_umap_Beta_emb_Beta_linear_index_grid_full_site.rds")


diffusion_density_plot_tsne <- diffusion_density_plot(umap_results[,1:2],vis_umap_Beta_emb_Beta_linear_index[[2]]$tp,forward = TRUE,2500,scale="log")

saveRDS(diffusion_density_plot_tsne,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/diffusion_density_plot_full_site.rds")

```

### vis
```{r warning= FALSE}
vis_umap_Beta_emb_Beta_linear_index_full_site <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/vis_umap_Beta_emb_Beta_linear_index_full_site.rds")

vis_umap_Beta_emb_Beta_linear_index_grid_full_site <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/vis_umap_Beta_emb_Beta_linear_index_grid_full_site.rds")

diffusion_density_plot_full_site <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/diffusion_density_plot_full_site.rds")

vis_umap_Beta_emb_Beta_linear_index_full_site[[1]]

vis_umap_Beta_emb_Beta_linear_index_grid_full_site[[1]]

diffusion_density_plot_full_site
```
...
vis_umap_Beta_emb_Beta_linear_index_random_list <- list()
vis_umap_Beta_emb_Beta_linear_index_grid_random_list <- list()

diffusion_density_plot_list <- list()
for (i in 1:100){
   print(i)
  index_site_name_random <- sample(rownames(res[[3]][["current"]]$M),length(index_site_name_int_intersect))
  library(VeloRM)
  arrow.scale=3; cell.alpha=0.4; cell.cex=1; fig.height=4;
  
  vis_umap_Beta_emb_Beta_linear_index  <- show.velocity.on.embedding.cor(umap_results[,1:2],
               res[[3]][["current"]]$M[index_site_name_random,index],
               res[[3]][["projected"]]$M[index_site_name_random,index],
               res[[3]][["delta"]]$M[index_site_name_random,index],
               cell.colors=cell.colors,n=200,scale='log',
               cex=cell.cex,arrow.scale=arrow.scale,arrow.lwd=0.5,rm_uniform="mode1")
  vis_umap_Beta_emb_Beta_linear_index_random_list[[i]] <- vis_umap_Beta_emb_Beta_linear_index
  ...
  vis_umap_Beta_emb_Beta_linear_index_grid <- show.velocity.on.embedding.cor(umap_results[,1:2],
               ...
               cell.colors=cell.colors,n=200,scale='log',
               cex=cell.cex,arrow.scale=arrow.scale,arrow.lwd=0.5,show.grid.flow = TRUE,grid.n = 20,rm_uniform="mode1")
  vis_umap_Beta_emb_Beta_linear_index_grid_random_list[[i]] <- vis_umap_Beta_emb_Beta_linear_index_grid
  diffusion_density_plot_tsne <- diffusion_density_plot(umap_results[,1:2],vis_umap_Beta_emb_Beta_linear_index[[2]]$tp,forward = TRUE,2500,scale="log")
  
  diffusion_density_plot_list[[i]] <- diffusion_density_plot_tsne
}


saveRDS(vis_umap_Beta_emb_Beta_linear_index_random_list,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/vis_umap_Beta_emb_Beta_linear_index_random_list.rds")
saveRDS(vis_umap_Beta_emb_Beta_linear_index_grid_random_list,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/vis_umap_Beta_emb_Beta_linear_index_grid_random_list.rds")

saveRDS(diffusion_density_plot_list,"D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/diffusion_density_plot.rds")
...
### origin vs all vs scvelo (pseudo time compare real time)
```{r warning=FALSE}
library(utils)
library(ggplot2)
library(dplyr)
...
# ================= 1. 数据加载 =================
scRNA_seq_1_exp_latent_umap <- read.csv("D:/data/VeloRM_data/scVelo/scRNA-seq_1/results/scRNA_seq_1_exp_latent_umap.csv")

diffusion_density_plot <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/diffusion_density_plot.rds")

diffusion_density_plot_full_site <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/diffusion_density_plot_full_site.rds")

# ================= 1. 数据加载 =================
meta <- scRNA_seq_1_exp_latent_umap
df1 <- diffusion_density_plot[["data"]]
df2 <- diffusion_density_plot_full_site[["data"]]
...
# ================= 3. pseudo =================
df1$pseudo_site <- norm01(df1$density)
df2$pseudo_full <- norm01(df2$density)

meta <- meta %>%
  mutate(cell = X,
         pseudo_scvelo = norm01(latent_time))

df_all <- meta %>%
  left_join(data.frame(cell = rownames(df1), pseudo_site = df1$pseudo_site),
            by = "cell") %>%
  left_join(data.frame(cell = rownames(df2), pseudo_full = df2$pseudo_full),
            by = "cell")
...
# lineage cells
df <- df_all %>%
  filter(phase %in% names(lineage_map)) %>%
  mutate(phase_num = lineage_map[phase])


calc_spearman <- function(pseudo, phase){
  abs(cor(pseudo, phase, method = "spearman", use = "complete.obs"))
}

# lineage 
res <- data.frame(
  Method = c("scVelo", "Site", "Full"),
  Spearman = c(
    calc_spearman(df$pseudo_scvelo, df$phase_num),
    calc_spearman(df$pseudo_site, df$phase_num),
    calc_spearman(df$pseudo_full, df$phase_num)
  )
)
...
### random select 100 time (pseudo time compare real time)
```{r warning=FALSE}
library(utils)
library(ggplot2)
library(dplyr)
# ================= 1. 数据加载 =================
scRNA_seq_1_exp_latent_umap <- read.csv(
  "D:/data/VeloRM_data/scVelo/scRNA-seq_1/results/scRNA_seq_1_exp_latent_umap.csv"
)

diffusion_density_plot <- readRDS("D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/diffusion_density_plot.rds")

diffusion_density_plot_list <- readRDS(
  "D:/data/VeloRM_data/scRNA-Seq_A-to-I_1/results/diffusion_density_plot_list.rds"
)
...
# ================= 4. 计算 100 次 random spearman =================
spearman_random <- sapply(diffusion_density_plot_list, function(obj){

  df_tmp <- obj[["data"]]

  # pseudo time
  df_tmp$pseudo <- norm01(df_tmp$density)

  # join
  df_merge <- meta %>%
    left_join(
      data.frame(cell = rownames(df_tmp), pseudo = df_tmp$pseudo),
      by = "cell"
    )

  # 计算 spearman
  calc_spearman(df_merge$pseudo, df_merge$phase_num)
})

# ================= 5. 整理结果 =================
res_random <- data.frame(
  Iteration = 1:length(spearman_random),
  Spearman = spearman_random
)

# summary
summary_res <- data.frame(
  Mean = mean(spearman_random, na.rm = TRUE),
  SD   = sd(spearman_random, na.rm = TRUE),
  Max  = max(spearman_random, na.rm = TRUE),
  Min  = min(spearman_random, na.rm = TRUE)
)
...
# ================= 6. baseline Spearman =================
df_origin <- diffusion_density_plot[["data"]]

# pseudo time
df_origin$pseudo <- norm01(df_origin$density)

# merge
df_origin_merge <- meta %>%
  left_join(
    data.frame(cell = rownames(df_origin), pseudo = df_origin$pseudo),
    by = "cell"
  )

# Spearman
spearman_origin <- calc_spearman(df_origin_merge$pseudo, df_origin_merge$phase_num)

print(paste("Baseline Spearman:", spearman_origin))
...
baseline_df <- data.frame(
  Iteration = NA,
  Spearman = spearman_origin,
  Type = "Origin"
)

plot_all <- bind_rows(plot_df, baseline_df)




method_levels <- c("Random", "Site", "Full", "scVelo")

# Random
random_df <- res_random %>%
  mutate(Method = factor("Random", levels = method_levels)) %>%
  dplyr::select(Method, Spearman)
...
# 合并数据（统一格式）
df_res <- bind_rows(
  res %>% dplyr::rename(Score = Spearman),
  random_df %>% dplyr::rename(Score = Spearman)
)

# 确保顺序
df_res$Method <- factor(df_res$Method,
                        levels = c("Random", "Site", "Full", "scVelo"))
...
p_stats <- ggplot(df_res, aes(x = Method, y = Score, fill = Method)) +
  ...
  # --- Random：box + jitter ---
  geom_boxplot(
    data = dplyr::filter(df_res, Method == "Random"),
    alpha = 0.5,
    width = 0.4,
    outlier.shape = NA,
    fill = "grey80"
  ...
  labs(
    ...
    subtitle = paste0("Random N=", nrow(random_df)),
    x = "",
  ...
  # 防止文字被截断
  expand_limits(y = max(df_res$Score) * 1.1)
